 ...
 * @author diogo
 */
public class CRUDUser extends LigacaoBD implements ICRUD {

    private static int id;

    public CRUDUser() throws ClassNotFoundException, SQLException {
        super();
        //statement.executeUpdate("drop table if exists users");
        //statement.executeUpdate("create table users (id integer, name string)");
        ResultSet rs = statement.executeQuery("select max (id) as id from users");
        while (rs.next()) {
            // read the result set
    ...
    public void create(String dado) {
        try {
            this.id++;
            statement.executeUpdate("insert into users values(" + this.id + ", '" + dado + "')");
